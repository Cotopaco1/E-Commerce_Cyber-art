
<div class="card_login contenedor">
    <div class="datos_login">
        <div class="datos_header">
            <!-- <h3>CyberArt</h3> -->
            <div class="logo"><a href="/"><img src="./img/logo_cyber-transparente.png" alt="imagenLogo"></a></div>

            <a href="/crear_cuenta">Crea una cuenta</a>
        </div>
        <div class="datos_body">
            <div class="datos_body_titulo">
                <h1>Cyber-Bienvenido!</h1>
                <p>Bienvenido al panel de inicio a nuestro tienda </p>
            </div>
            <div class="datos_body_inputs">

                <form class="formulario" id="formulario">
                    <input autocomplete="email" name="email" type="email" class="input" id="email" placeholder="Tu email">
                    <input autocomplete="current-password" name="password" type="password" class="input" id="password" placeholder="Tu contraseña">
                </form>
                <a href="/recuperar_password" class="link_olvidaste_password">¿Olvidaste la contraseña?</a>
            </div>
        </div>
        <div class="datos_footer">
            <boton class="botonNegro boton_login" id="boton_ingresar">Ingresar</boton>
            <p>No tienes una cuenta? <a href="/crear_cuenta">Registrate</a></p>
        </div>
    </div>
    <div class="imagen_card_login"></div>
</div>